# Изумруд (Emerald) — custom BLE keyboard
...
2. **Pair** *(first time or new device only)*
   Hold the **top-left key (Tab)** and **bottom-left key (Ctrl)** simultaneously
   until the LEDs light up. The keyboard is now discoverable. Pair from your
   computer or phone, then release the keys. The LEDs turn off and the keyboard
   locks back to bonded devices only.
...
New devices can only pair while the **Tab + Ctrl combo is held** (LEDs on).
